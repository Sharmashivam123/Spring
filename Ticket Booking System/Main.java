...
public class Main {
	static Integer pin;static int id;
	...
	public static void main(String[] args)throws IOException {
		...
		db.getLocation().stream().map(location->location.getPin()).forEach(pin->System.out.print(pin+" "+"\n"));
		console.info("Enter the pin : ");
			...
			if( moviesAtLocation == null )throw new Exception("Invalid movie name");	
			else
			{
				moviesAtLocation.stream().map(movie->movie.getMovieName()).forEach( movie -> System.out.print(movie+" "));
				System.out.println();
			}
			...
			if(theatreByMovie == null) throw new Exception("Invalid Input");
			for(Theatre t:theatreByMovie)System.out.println(t.getTheatreId() + " " + t.getTheatreName() + "  " + t.getMovieTime());
